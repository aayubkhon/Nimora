...
class Follow {
  ...
  async subscribeData(member, data) {
    try {
      ...
      await this.modifyMemberFolowCounts(follow_id, "subscriber_change", 1);
      await this.modifyMemberFolowCounts(subscriber_id, "follow_change", 1);
      return true;
    } catch (err) {
      throw err;
    }
  ...
  async modifyMemberFolowCounts(mb_id, type, modifier) {
    try {
  ...
  async unsubscriberData(member, data) {
    try {
      ...
      assert.ok(result, Definer.general_err1);
      await this.modifyMemberFolowCounts(follow_id, "subscriber_change", -1);
      await this.modifyMemberFolowCounts(subcriber_id, "follow_change", -1);
      return true;
    } catch (err) {
      throw err;
    }
  ...
  async getMembeFollowersData(member, inquiry) {
    try {
      ...
      // following followed back t subscriber
